...
def one_user(user_id):
    ...
    # return json.dumps([user.to_dict() for user in User.query.all()]), 200
        ...
        db.session.commit()
        db.session.close()
        return f"Объект заказ с id {user_id} успешно изменен!", 200
        ...
        db.session.commit()
        db.session.close()
        return f"Объект заказ с id {user_id} успешно удален!", 200
...
def one_order(order_id):
    if request.method == 'GET':
        ...
        db.session.commit()
        db.session.close()
        return f"Объект заказ с id {order_id} успешно изменен!", 200
    elif request.method == 'DELETE':
        order = db.session.query(Order).get(order_id)
        if order is None:
            return "Заказ не найден!", 404
        ...
        db.session.commit()
        db.session.close()
        return f"Объект заказ с id {order_id} успешно удален!", 200
...
def one_offer(offer_id):
    if request.method == 'GET':
        ...
        db.session.commit()
        db.session.close()
        return f"Объект предложение с id {offer_id} успешно изменен!", 200
        ...
        db.session.commit()
        db.session.close()
        return f"Объект предложение с id {offer_id} успешно удален!", 200
